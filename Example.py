...
greeting = 'Hi '
name = 'Emmanuel'  # enter your name in this string
...
age = 21 # enter your age here (as a number)
...
age = '21' # enter your age here, as a string
...
my_class.extend(['jaime', 'pedro', 'pirry'])
...
splited_long_sentence = my_long_sentence.split(' ')

print len(splited_long_sentence)
...
for i in students:
    print(i[0] + ' is ' + str(i[1]) + ' years old')
...
tuples = [
    ('David', 12, 'Matematicas', 'futbol'),
    ('Sofia', 13, 'biologia', 'tennis'),
    ('Samuel', 12, 'geografia', 'football'),
    ('Kathia', 11, 'musica', 'beisbol'),
    ('Daniel', 10, 'fisica', 'ajedrez')
]

# Now loop over them printing each one out

for nombres in tuples:
    print(nombres[0] + ' tiene ' + str(nombres[1]) + ' anos y su materia favorita es ' + nombres[2] + ' y su deporte favorito es el ' + nombres[3])

# Now pick a number (in the students' age range)
# Make the loop only print the students older than that number

for mayores in tuples:
    if mayores[1] > 11:
        print(mayores[0] + ' es mayor a 11 anos y su materia favorita es ' + mayores[2] + ' y su deporte favorito es el ' + mayores[3])
